...
const CreatePost: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Erro ao criar post. Tente novamente.')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <Container className="animate-fade-in">
      ...
      <Title className="animate-slide-up">Criar Publicação</Title>
      ...
      <Card className="animate-slide-up hover:shadow-2xl transition-shadow duration-300">
        <form onSubmit={handleSubmit}>
          ...
          {success && <SuccessMessage className="animate-fade-in">Post criado com sucesso! Redirecionando...</SuccessMessage>}
